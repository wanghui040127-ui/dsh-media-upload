...
/** Render one image block as a model-visible text placeholder (see-skill contract). */
function imagePlaceholder(block) {
	const ref = block.attachment;
	if (ref === void 0) return "[用户上传的图片附件]";
	const hash = String(ref.attachmentId).replace(/^sha256:/, "");
	const meta = [ref.mediaType, `${ref.width}x${ref.height}`, ref.name].filter(Boolean).join(", ");
	return `[用户上传的图片附件: sha256:${hash} (${meta})]`;
}
/** Render one generic file block as a model-visible text placeholder (PDF etc.). */
function filePlaceholder(block) {
	const ref = block.attachment;
	if (ref === void 0) return "[用户上传的文件附件]";
	const hash = String(ref.attachmentId).replace(/^sha256:/, "");
	const meta = [ref.mediaType, ref.name].filter(Boolean).join(", ");
	return `[用户上传的文件附件: ${ref.mediaType} sha256:${hash} (${meta})]`;
}
/** Flatten user-role content, converting image and file blocks into text placeholders. */
function flattenContent(blocks) {
	return blocks.map((block) => block.type === "text" ? block.text : block.type === "image" ? imagePlaceholder(block) : block.type === "file" ? filePlaceholder(block) : "").join("");
}
/** Reject image content on assistant/system output paths where no wire representation exists. */
function assertNoImageOutput(blocks) {
...
function serializeMessages(messages) {
	...
	for (const message of messages) {
		if (message.role !== "user") assertNoImageOutput(message.content);
