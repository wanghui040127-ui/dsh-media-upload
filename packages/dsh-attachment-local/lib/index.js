...
/**
* Save and verify immutable generic file bytes (non-image) below a versioned
* attachment root, reusing the same content-addressed storage as images but
* skipping raster decode. Only the byte length and digest are checked.
* @param root - absolute `DSH_HOME/attachments/v1` root.
* @param input - encoded bytes and declared metadata.
* @param maxBytes - deployment's per-file byte limit.
* @returns durable content-addressed reference.
*/
async function saveFileObject(root, input, maxBytes) {
	if (input.data.byteLength === 0) throw new AttachmentError("File is empty.", "INVALID_FILE");
	if (input.data.byteLength > maxBytes) throw new AttachmentError("File exceeds the configured byte limit.", "FILE_TOO_LARGE");
	const sha256 = digest(input.data);
	const bucket = join(root, "objects", sha256.slice(0, 2));
	const staging = join(root, "tmp");
	const boundary = await ensureDurableHome(dirname(dirname(resolve(root))));
...
* @returns verified bytes and reference.
*/
async function readFileObject(root, ref, signal) {
	signal?.throwIfAborted();
	const sha256 = ensureReference(ref);
	let data;
	try {
...
/** Default maximum files in one prompt. */
const DEFAULT_MAX_FILES_PER_MESSAGE = 20;
/** Default maximum aggregate file bytes in one prompt. */
const DEFAULT_MAX_MESSAGE_FILE_BYTES = 200 * 1024 * 1024;
...
var LocalAttachmentStore = class extends AttachmentStore {
		...
		constructor(ctx, config) {
			...
			this.fileLimits = Object.freeze({
				maxFileBytes: config.maxFileBytes ?? 52428800,
		...
		async saveFile(input) {
			return saveFileObject(this.root, input, this.fileLimits.maxFileBytes);
		}
	...
	//#endregion
	export { DEFAULT_MAX_IMAGES_PER_MESSAGE, DEFAULT_MAX_IMAGE_BYTES, DEFAULT_MAX_IMAGE_PIXELS, DEFAULT_MAX_MESSAGE_IMAGE_BYTES, DEFAULT_MAX_FILE_BYTES, DEFAULT_MAX_FILES_PER_MESSAGE, DEFAULT_MAX_MESSAGE_FILE_BYTES, LocalAttachmentStore, LocalAttachmentStore as default, detectImage, readImageFile, saveImageFile, validateImageFile, saveFileObject, readFileObject };
