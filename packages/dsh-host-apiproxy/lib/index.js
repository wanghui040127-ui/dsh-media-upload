...
const imageMediaTypeSchema = z$1.union([z$1.literal("image/png"), z$1.literal("image/jpeg"),
	z$1.literal("image/webp"),
	z$1.literal("image/gif")
]);
/** Any MIME type accepted for a generic file attachment. */
const fileMediaTypeSchema = z$1.string().min(1);
...
async function durablePromptContent(ctx, content) {
	...
	if (imageParts.length > imageLimits.maxImagesPerMessage) throw new AttachmentError("Prompt exceeds the configured image-count limit.", "TOO_MANY_IMAGES");
	if (fileParts.length > fileLimits.maxFilesPerMessage) throw new AttachmentError("Prompt exceeds the configured file-count limit.", "TOO_MANY_FILES");
	const prepared = content.map((part) => part.type === "text" ? part : {
		part,
		data: decodeBase64(part.data)
	});
	const images = prepared.filter((part) => "data" in part && part.part.type === "image");
	const files = prepared.filter((part) => "data" in part && part.part.type === "file");
	if (images.reduce((sum, image) => sum + image.data.byteLength, 0) > imageLimits.maxMessageImageBytes) throw new AttachmentError("Prompt exceeds the configured aggregate image-byte limit.", "IMAGES_TOO_LARGE");
	if (files.reduce((sum, file) => sum + file.data.byteLength, 0) > fileLimits.maxMessageFileBytes) throw new AttachmentError("Prompt exceeds the configured aggregate file-byte limit.", "FILES_TOO_LARGE");
	...
	return {
		sessions: {
			...
			async selectModel(request) {
				...
				return serializeImageAdmission(found.agent, async () => {
					try {
						...
						// No inputModalities gate on model switches: sessions containing
						// images stay usable with text-only adapters (see-skill placeholder
						// flattening) and with vision adapters (native image blocks).
						const selected = {
			...
			async prompt(request) {
				...
				const admit = async () => {
					try {
						// Image uploads are admitted for every model: text-only adapters
						// flatten image blocks into see-skill placeholders, vision adapters
						// receive the native blocks. No inputModalities gate here.
		...
		host: {
			...
			async openExternal(request, signal) {
				...
				try {
					await openNativePath(url, signal);
					return ok(request, { opened: true });
				}
				catch (error) {
					return err(request, {
		...
		balance: {
			async get(request) {
				...
				try {
					...
					if (hit === void 0 || hit.value.length === 0) return err(request, {
						code: "missing-credential",
						message: "no DeepSeek API key configured; store DEEPSEEK_API_KEY through the web Models page",
...
/** host.openExternal request payload: an absolute http(s) URL to open in the user's browser. */
const hostOpenExternalRequestSchema = z$1.object({ url: z$1.string().url().regex(/^https?:\/\//i) });
/** host.openExternal response value. */
const hostOpenExternalValueSchema = z$1.object({ opened: z$1.literal(true) });
//#endregion
...
//#endregion
//#region lib/types/api/balance.schema.js
/**
* balance domain zod schemas (names derived from map keys: balanceGetRequestSchema /
* balanceGetValueSchema). The value mirrors the DeepSeek user-balance endpoint
* (`GET https://api.deepseek.com/user/balance`) field-for-field.
*/
/** One currency line of the DeepSeek user-balance response. */
const balanceEntrySchema = z$1.object({
	currency: z$1.string(),
	total_balance: z$1.string(),
	granted_balance: z$1.string(),
	topped_up_balance: z$1.string()
});
/** balance.get request payload. */
const balanceGetRequestSchema = z$1.object({});
/** balance.get response value. */
const balanceGetValueSchema = z$1.object({
	is_available: z$1.boolean(),
	balance_infos: z$1.array(balanceEntrySchema)
});
//#endregion
...
const UNARY_ROUTES = {
	...
	"host.openExternal": {
		schema: hostOpenExternalRequestSchema,
		invoke: (api, r, signal) => api.host.openExternal(r, signal)
	},
	"workspace.list": {
	...
	"balance.get": {
		schema: balanceGetRequestSchema,
		invoke: (api, r) => api.balance.get(r)
	},
	"llm.providers": {
